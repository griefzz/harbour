        ...
        namespace aes256 {
            ...
            auto encrypt(RandomAccessScalarRange auto &&plaintext,
                ...
                if (!EVP_EncryptInit_ex(ctx.get(), cipher, nullptr,
                                        ...
                                        reinterpret_cast<const u8 *>(iv.data()))) {
                    return {};
                }

                if (!EVP_EncryptUpdate(ctx.get(),
                                       reinterpret_cast<u8 *>(ciphertext.data()),
                                       &len,
                                       reinterpret_cast<const u8 *>(plaintext.data()),
                                       plaintext.size())) {
                    return {};
                }
                int ciphertext_len = len;

                if (!EVP_EncryptFinal_ex(ctx.get(), reinterpret_cast<u8 *>(ciphertext.data() + len), &len)) {
                    return {};
                }
                ciphertext_len += len;
            ...
            auto decrypt(RandomAccessScalarRange auto &&ciphertext,
                ...
                if (!EVP_DecryptUpdate(ctx.get(),
                                       reinterpret_cast<u8 *>(plaintext.data()),
                                       &len,
                ...
                if (!EVP_DecryptFinal_ex(ctx.get(),
                                         reinterpret_cast<u8 *>(plaintext.data()) + len,
                                         &len)) {
                    return {};
                }
                plaintext_len += len;
